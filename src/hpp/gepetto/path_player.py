...
class PathPlayer(object):
    ...
    def displayPath(self, pathId, color=[0.85, 0.75, 0.15, 0.9], jointName=0):
        ...
        self.publisher.client.gui.addToGroup(nameCurve,
                                             self.publisher.sceneName)
        self.publisher.client.gui.refresh()
    ...
    def displayVelocityPath(self, pathId, jointName=0):
        if self.client.robot.getDimensionExtraConfigSpace() < 6:
            raise RuntimeError(
                "DisplayVelocityPath can only be used if the robot have at least 6 extraDof storing velocity and acceleration of the root."
            )
        if jointName == 0:
            if self.publisher.robot.rootJointType == 'planar':
                jointName = self.publisher.robot.tf_root + '_joint'
        vmax = omniORB.any.from_any(
            self.client.problem.getParameter(
                "Kinodynamic/velocityBound")) * math.sqrt(2)
        configSize = self.client.robot.getConfigSize(
        ...
        while t < length:
            ...
            if last_q != 0:
                ...
                v = (math.sqrt(q[configSize] * q[configSize] +
                               q[configSize + 1] * q[configSize + 1] +
                               q[configSize + 2] * q[configSize + 2])) / vmax
                if v < 0.5:
                    color = [v * 2., 1, 0, 1]
                else:
                    color = [1, 2 - (v * 2.), 0, 1]
